    ...
    const Order_Detail = () => {
        ...
        useEffect(() => {
            const fetchData = async () => {
                try {
                    const response = await fetch('http://192.168.2.134:8000/api/order-details/');
                    const response_data = await response.json();
                    const filteredData = response_data.filter(item => {
                        return (item.user_id == 1 && item.number == orderId);
                    });
                    
                    setData(filteredData[0]);
                    console.log(filteredData[0].basket);
               
    
                    const shippingAddressResponse = await fetch('http://192.168.2.134:8000/api/shipping-address/');
                    const addressData = await shippingAddressResponse.json();
                    const shippingAddress = addressData.filter(item => {
                        return (item.id == filteredData[0].shipping_address_id);
                    });
    
                    setShippingAddress(shippingAddress);
                    console.log("sycfgeysdgvuj",shippingAddress);

                    const productDetail = await fetch('http://192.168.2.134:8000/api/product-detail/');
                    const productDetailResponse = await productDetail.json();
                    
                    setProductDetail(productDetailResponse)

                    const productImage = await fetch('http://192.168.2.134:8000/api/product-image/');
                    const ProductImageResponse = await productImage.json();
        ...
        const renderMobileItems = () => {
            if (data && data.basket) {
                return data.basket.map((product, index) => {
                    const productDetails = productDetail.find(detail => detail.id === product.product_id);
                    const productImageDetails = productImage.find(image => image.product === product.product_id);
                    if (productDetails) {
                        ...
                        return (
                            <OrderDetailMobile
                                ...
                                item_image={productImageDetails ? productImageDetails.original : ''}
                                item_price={productDetails.prices[0].price}
                                sub_total={`$${parseFloat(productDetails.originalPrice).toFixed(2)}`}
                                qty={product.quantity}
                                subtotal={product.price_excl_tax}
                            />
                        );
                    }
                ...
                return null; // Return null if data or data.basket is undefined
            }
        };
                    ...
                    <div className="Order_Detail border-[1px] border-[#E6E6E6] w-full rounded-lg">
                        ...
                        <div className="md:px-6 px-3 grid grid-cols-12 md:gap-6 gap-3 md:py-9 py-5">
                            ...
                            <div className="lg:col-span-8 sm:col-span-6 col-span-12 grid grid-cols-12 border-[1px] border-[#E6E6E6] rounded-lg divide-y-[1px] md:divide-x-[1px] divide-[#E6E6E6] ">
                                ...
                                {shippingAddress.map((shippingAddressItem, index) => (

                                <div className="lg:col-span-6 col-span-12">
                                    <h1 className="text-[#999] text-sm border-b-[1px] border-[#E6E6E6] md:py-[18px] py-2.5 px-5 ">SHIPPING ADDRESS</h1>
                                    <p className="text-base pt-3.5 px-5 ">{shippingAddressItem.first_name} {shippingAddressItem.last_name}</p>
                                    <p className="text-sm text-[#666] px-5 pt-2">4140 Parker Rd. Allentown, New Mexico 31134</p>
                                    <p className="text-xs text-[#999] px-5 md:pt-9 pt-5">EMAIL</p>
                                    <p className="text-sm px-5 ">dainne.ressell@gmail.com</p>
                                    <p className="text-xs text-[#999] px-5 pt-2">PHONE</p>
                                    <p className="text-sm  px-5 pb-5">{shippingAddressItem.phone_number}</p>
                                </div>
                                ...
                                <div className="border-[1px] border-[#E6E6E6] rounded-lg ">
                                    <div className=" py-[18px] px-5 flex lg:flex-row flex-col w-full justify-between lg:divide-y-0 divide-y-2 lg:divide-x-2 divide-[#E6E6E6] border-b-[1px] border-[#E6E6E6]">
                                        <div className="flex lg:flex-col flex-row justify-between">
                                            ...
                                            <p className="text-sm text-[#1A1A1A] lg:pt-2.5">{ orderId }</p>
                                        </div>
                                    ...
                                    <div className="divide-y-2 py-[18px] px-5">
                                        <div className="subtotal py-2 ssm:py-3 flex justify-between text-sm">
                                            ...
                                            <span className="text-[#1A1A1A] text-end">${data.total_incl_tax}</span>
                                        </div>
                                        ...
                                        <div className="total py-2 ssm:py-3 flex justify-between text-lg">
                                            ...
                                            <span className="font-semibold text-[#E6992A]">${data.total_incl_tax}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
