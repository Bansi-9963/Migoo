...
const MyWishListItem = props => {
  ...
  return (
    <tr
      className={`bg-white border-b  hover:bg-gray-50 text-gray-900 text-base font-normal leading- ${
        isOutOfStock ? 'out-of-stock-style' : ''
      }`}
    >
      <th
        scope='row'
        className='px-6 sm:px-6 py-2.5 whitespace-nowrap  flex lg:flex-row flex-col-reverse'
      >
        <img
      ...
      <td className='px-2 sm:px-6 lg:px-12 xl:px-20 py-4  self-center'>
        ...
        <span className='text-gray-400 text-base font-normal leading-6 line-through'>
          {props.item_price2}
        </span>
      </td>
      ...
      <td className='px-2 sm:px-4    py-4 text-right self-center'>
        <div className='flex gap-5'>
          <div
            className={`px-4 md:px-8 py-3.5 btn text-white rounded-full ${
              isOutOfStock
